...
def report(state: InvestigationState) -> InvestigationState:
    ...
    report_prompt = (
        ...
        f"---\n\n"
        f"LANGUAGE: Write entirely in English regardless of the language of the user question.\n\n"
        f"FORMATTING: Use bold (**...**) for the root cause only — the single dominant "
        f"driver that best answers the user's question. Do NOT bold supporting evidence, "
        f"secondary findings, section headers within your prose, or any other text.\n\n"
        ...
        f"gap is audience selection, not campaign quality'). Bold the root cause "
        f"phrase only. Then give the supporting numbers — the controlled "
        f"comparison and the residual — and what they mean. If the investigation "
        f"reframed the user's question, say so here.\n\n"
