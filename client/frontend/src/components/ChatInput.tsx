...
interface Props { onSubmit: (question: string) => void; disabled: boolean; }

export default function ChatInput({ onSubmit, disabled }: Props) {
  const [value, setValue] = useState("");
  const submit = () => { const q = value.trim(); if (!q || disabled) return; onSubmit(q); setValue(""); };

  return (
    <div className={`flex items-center rounded-xl overflow-hidden border transition-colors ${
      disabled ? "border-frame" : "border-frame focus-within:border-violet-500"
    } bg-surface`}>
      <span className="pl-4 pr-2 font-mono text-violet-400 text-sm select-none">❯</span>
      <textarea
        value={value}
