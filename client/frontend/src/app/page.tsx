      ...
      await runInvestigation(
        ...
        (evt) => {
          setHistory((h) => {
            ...
            const u = { ...cur };
            if (evt.type === "phase")
              u.phases = [...new Set([...u.phases, evt.phase as Phase])];
            else if (evt.type === "evidence") {
              const { type: _, ...item } = evt;
              u.evidence = [...u.evidence, item as EvidenceItem];
            } else if (evt.type === "report") {
              const { type: _, ...r } = evt;
              u.phases = [
                ...new Set([...u.phases, "critique", "report"]),
              ] as Phase[];
            ...
            <div key={`${entry.startedAt}-${i}`} className="space-y-4">
              ...
              <div className="flex justify-end">
                <div className="max-w-2xl">
                  <p className="text-xs text-violet-400 text-right mb-1.5 font-mono uppercase tracking-wider">
                    Query
                  </p>
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-surface border-b border-frame">
                  ...
                  >
                    {entry.streaming ? (
                      <>
                        <ThinkingDots />
                        <span className="text-xs font-mono text-violet-300 uppercase tracking-widest ml-1">
                          Investigating
                        </span>
                      </>
                    ) : entry.interrupted ? (
                ...
                <div className={`bg-bg p-5 ${entry.streaming ? "space-y-3" : "space-y-5"}`}>
                  {entry.phases.length > 0 && (
                    <PhaseTimeline
                      currentPhase={
                        entry.streaming
                          ? entry.phases[entry.phases.length - 1]
                          : null
                      }
                      completedPhases={entry.phases}
                    />
                  )}
